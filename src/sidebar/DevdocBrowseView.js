...
class BrowseView extends Gtk.ScrolledWindow {
  ...
  #createBrowseSelectionModel() {
    ...
    this.selection_model.connect("selection-changed", () => {
      ...
      if (this.selection_model.selected_item.item.html) {
        this.webview.load_html(
          this.selection_model.selected_item.item.html,
          null,
        );
        return;
      }
  ...
  async #loadDocs() {
    await this.#readIndex(
      Gio.File.new_for_path("src/sidebar/db.json"),
      Gio.File.new_for_path("src/sidebar/index.json"),
      "JavaScript",
    );
    this.emit("browse-view-loaded");
  }

  async #readIndex(dbjson, indexjson, docset_name) {
    let [data] = await indexjson.load_contents_async(null);
    const index = JSON.parse(decode(data));

    [data] = await dbjson.load_contents_async(null);
    const db = JSON.parse(decode(data));

    const doc = new DocumentationPage({
      name: docset_name,
      search_name: docset_name,
      uri: `index.html`,
      html: db["index"],
      children: this.#newListStore(),
    });
    this.root_model.insert_sorted(doc, this.#sortFunc);
    const types = {};
    for (const type of index.types) {
      const type_doc = new DocumentationPage({
        name: type.name,
        search_name: type.name,
        // html: db[type.slug],
        children: this.#newListStore(),
      });
      types[type.name] = type_doc;
      doc.children.insert_sorted(type_doc, this.#sortFunc);
    }
    for (const item of index.entries) {
      const item_doc = new DocumentationPage({
        name: item.name,
        search_name: item.name,
        uri: item.path,
        html: db[item.path],
      });
      types[item.type].children.insert_sorted(item_doc, this.#sortFunc);
    }
  }
